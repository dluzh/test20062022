        ...
        <form method="post" action="{{ route('entry.update', ['entry' => $entry->id]) }}">
            ...
            <div class="form-group m-3">
                ...
                <input type="text" class="form-control" name="title" placeholder="Title"
                    required maxlength="100" value="{{ old('title') ?? $entry->title ?? '' }}">
            </div>
            
            <div class="form-group m-3">
                <label for="title">Link</label>
                <input type="text" class="form-control" name="link" placeholder="Link"
                    required maxlength="100" value="{{ old('link') ?? $entry->link ?? '' }}">
            </div>

            <div class="form-group m-3">
                <input type="hidden" class="form-control" name="description" placeholder="description"
                    required maxlength="100">
                    <textarea name="description" id="myEditor">{!! old('description') ?? $entry->description ?? '' !!}</textarea>
